...
uint8_t ModbusMaster::writeMultipleRegisters(uint16_t u16WriteAddress,
  uint16_t u16WriteQty, uint16_t data[11])  //, uint16_t data1, uint16_t data2)
{
  _u16WriteAddress = u16WriteAddress;
  _u16WriteQty = u16WriteQty;
	for(int i=0;i<11;i++)
	{
  _u16TransmitBuffer[i] = data[i];
	}
...
void ModbusMaster::slaves(uint8_t slave_id)
{
//  txBuffer = (uint16_t*) calloc(ku8MaxBufferSize, sizeof(uint16_t));
  _u8MBSlave = slave_id;
	//Serial.println( _u8MBSlave);
