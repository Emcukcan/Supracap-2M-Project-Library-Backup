...
#define T35  5
#define  MAX_BUFFER  1024	//!< maximum size for the communication buffer in bytes
...
private:
    ...
    uint8_t u8regsize;
    uint8_t send_data_buff[MAX_BUFFER];
...
public:
    ...
    int8_t receive(uint16_t *rec_data) ;
    //
...
int8_t Modbus::receive(uint16_t *rec_data )  
{
    // check if there is any incoming frame
  uint8_t a=0;
	  ...
	  for(int i=0;i<20;i++)
	{
	  ...
	  rec_data[i]= (au8Buffer[a]<<8+ au8Buffer[a+1]);
		Serial.println(rec_data[i]);
...
//Serial.println(u8BufferSize);
   u8current=u8BufferSize;
    ...
    if (u8current == 0) return 0;
//		Serial.println("2efgff");
    //Serial.println( u8id ,HEX);
    // check T35 after frame end or still no frame end
...
//    }
    ...
    u8lastRec = 0;
//			Serial.println("3efgff");
   // int8_t i8state = getRxBuffer();
		//Serial.println( i8state ,HEX);
		 // Serial.println( au8Buffer[ 1 ]);
    u8lastError = i8state;
    if (i8state < 7) return i8state;
//			Serial.println("4efgff");
//	  Serial.println("u8current");
//    // check slave id
//	 Serial.println( u8id);
//		 Serial.println( au8Buffer[ID ]);

    if (au8Buffer[ ID ] != u8id) return 0;
// Serial.println( "au8Buffer[ID ]");
//    Serial.println( au8Buffer[ID ]);
//		Serial.println( au8Buffer[1],HEX);
//		Serial.println( au8Buffer[2],HEX);
//		Serial.println( au8Buffer[3],HEX);
//		Serial.println( au8Buffer[4],HEX);
//		Serial.println( au8Buffer[5],HEX);
//		Serial.println( au8Buffer[6],HEX);
//		Serial.println( au8Buffer[7],HEX);
				
//						Serial.println("u8BufferSize");
//Serial.println( au8Buffer[ID ]);
    // validate message: CRC, FCT, address and size
    uint8_t u8exception = validateRequest();
	//Serial.println("1efgff");
    ...
    if (u8exception > 0)
    {
        if (u8exception != NO_REPLY)
        {
            buildException( u8exception );
					//	Serial.println(au8Buffer[ 0 ],HEX);
            sendTxBuffer();
        }
...
//	Serial.println("1efgff");
    u32timeOut = millis();
    u8lastError = 0;
    //Serial.println( au8Buffer[ FUNC ] ,HEX);
    // process message
    switch( au8Buffer[ FUNC ] )
    {
    case MB_FC_READ_COILS:
    case MB_FC_READ_DISCRETE_INPUT:
      ...
      case MB_FC_READ_INPUT_REGISTER:
		//	return process_FC4( regs, u8size );
     //   break;
...
//		}
		 ...
		 u8BufferSize= read_serial(au8Buffer);
...
//		Serial.println( au8Buffer[ 6 ],HEX);
//		Serial.println( au8Buffer[ 7 ],HEX);
//		Serial.println( au8Buffer[ 8 ],HEX);
//		Serial.println( au8Buffer[ 9 ],HEX);
//		Serial.println( au8Buffer[ 10 ],HEX);
//		Serial.println( au8Buffer[ 11 ],HEX);
//		Serial.println( au8Buffer[ 12 ],HEX);
//		Serial.println( au8Buffer[ 13 ],HEX);
//		Serial.println( au8Buffer[ 14 ],HEX);
//		Serial.println( au8Buffer[ 90 ],HEX);
...
//    Serial.println(au8Buffer[ 13 ] ,HEX);
//    Serial.println(au8Buffer[ 14] ,HEX);
	//	Serial.println(au8Buffer[ 178 ] ,HEX);
    if (u8txenpin > 1)
    {
				//Serial.println("efef");
  ...
  //  port->write( au8Buffer, u8BuffbufgerSize );
   write_serial((char*)au8Buffer,u8BufferSize);
//    Serial.println(au8Buffer[ 0 ] ,HEX);
 // Serial.println("efgff");

   // uart_write_bytes(UART_NUM_2,(char*)au8Buffer, u8BufferSize);
...
uint8_t Modbus::validateRequest()
{
    ...
    case MB_FC_WRITE_MULTIPLE_REGISTERS :
        ...
        u8regs = (uint8_t) u16regs;
			//Serial.println(au8Buffer[ 0 ],HEX);
		//Serial.println(u8regs,HEX);
		//Serial.println(u8regsize,HEX);
...
int8_t Modbus::process_FC3( uint16_t *regs, uint8_t /*u8size*/ )
{
    ...
    uint16_t i;
   // Serial.println(au8Buffer[ NB_HI ],HEX);
	 // Serial.println(au8Buffer[ NB_LO ],HEX);
   ...
   // Serial.println(au8Buffer[ 3 ] ,HEX);
    ...
    {
			  	//send_data_buff[ u8BufferSize ] = highByte(regs[i]);
			// Serial.println(u8StartAdd ,HEX);
        au8Buffer[ u8BufferSize ] = highByte(regs[i]);
			// Serial.println(i );
		  // Serial.println(highByte(regs[i]) ,HEX);
		   //  Serial.println(au8Buffer[ u8BufferSize ],HEX);
			 // Serial.println( u8BufferSize ,HEX);
        u8BufferSize++;
		//	send_data_buff[ u8BufferSize ] = lowByte(regs[i]);
       au8Buffer[ u8BufferSize ] = lowByte(regs[i]);
			//Serial.println(au8Buffer[ u8BufferSize ],HEX);
        u8BufferSize++;
    }
    u8CopyBufferSize = u8BufferSize +2;
		//Serial.println(send_data_buff[ 3 ] ,HEX);
	 // Serial.println(au8Buffer[ 4 ] ,HEX);
	//  Serial.println(send_data_buff[ 5 ] ,HEX);
 //  Serial.println(au8Buffer[ 30] ,HEX);
  // Serial.println(au8Buffer[ 25 ] ,HEX);
...
//temp=au8Buffer[  (BYTE_CNT + 1) + i * 2 ] << 8 | au8Buffer[ (BYTE_CNT + 2) + i * 2 ];
